...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-Scribe"
  !define PRODUCT_WEB_SITE "https://github.com/wakiyamap/electrum-scribe"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-scribe-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-scribe\electrum_scribe\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-scribe\*.*"
  File "c:\electrum-scribe\electrum_scribe\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-scribe-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-scribe-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-scribe-${PRODUCT_VERSION}.exe" 0
  ;CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-scribe-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-scribe-${PRODUCT_VERSION}.exe" 0


  ;Links scribe: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\scribe" "" "URL:scribe Protocol"
  WriteRegStr HKCU "Software\Classes\scribe" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\scribe" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\scribe\shell\open\command" "" "$\"$INSTDIR\electrum-scribe-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\scribe"
