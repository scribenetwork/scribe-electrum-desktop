...
version=`python3 -c "import electrum_scribe; print(electrum_scribe.version.ELECTRUM_VERSION)"`
...
for mine in $(ls dist/*.exe); do
    echo "---------------"
    f=$(basename $mine)
    echo "Downloading https://scribe.network/$version/$f"
    wget -q https://scribe.network/$version/$f -O signed/$f
