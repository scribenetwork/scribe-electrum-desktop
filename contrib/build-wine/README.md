...
2. Build image
    ...
    ```
    $ sudo docker build -t electrum-scribe-wine-builder-img contrib/build-wine
    ```
...
3. Build Windows binaries
    ...
    $ FRESH_CLONE=contrib/build-wine/fresh_clone && \
        ...
        mkdir -p $FRESH_CLONE && \
        cd $FRESH_CLONE  && \
        git clone https://github.com/scribenetwork/scribe-electrum-desktop.git && \
        cd electrum-scribe
    ```
    ...
    $ git checkout $REV
    $ sudo docker run -it \
        --name electrum-scribe-wine-builder-cont \
        -v $PWD:/opt/wine64/drive_c/electrum-scribe \
        --rm \
        --workdir /opt/wine64/drive_c/electrum-scribe/contrib/build-wine \
        electrum-scribe-wine-builder-img \
        ./build.sh
    ```

    ```
    sudo docker run -it         --name electrum-scribe-wine-builder-cont         -v $PWD:/opt/wine64/drive_c/electrum-scribe         --rm         --workdir /opt/wine64/drive_c/electrum-scribe/contrib/build-wine         tolak/electrum-wine-builder-img         ./build.sh
    ```
