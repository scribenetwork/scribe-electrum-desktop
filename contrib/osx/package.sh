...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-Scribe.app"
    exit -127
fi
...
dmg dmg Electrum_uncompressed.dmg electrum-scribe-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum_uncompressed.dmg

echo "Done."
sha256sum electrum-scribe-$VERSION.dmg
