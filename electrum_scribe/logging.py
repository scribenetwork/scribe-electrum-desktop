...
def _shorten_name_of_logrecord(record: logging.LogRecord) -> logging.LogRecord:
    ...
    if record.name.startswith("electrum_scribe."):
...
electrum_logger = logging.getLogger("electrum_scribe")
electrum_logger.setLevel(logging.DEBUG)


def _delete_old_logs(path, keep=10):
    files = sorted(list(pathlib.Path(path).glob("electrum_scribe_log_*.log")), reverse=True)
    for f in files[keep:]:
        os.remove(str(f))
...
def _configure_file_logging(log_directory: pathlib.Path):
    ...
    PID = os.getpid()
    _logfile_path = log_directory / f"electrum_scribe_log_{timestamp}_{PID}.log"
...
def get_logger(name: str) -> logging.Logger:
    if name.startswith("electrum_scribe."):
        name = name[14:]
...
def configure_logging(config):
    ...
    _logger.info(f"Electrum version: {ELECTRUM_VERSION} - https://scribe.network.org - {GIT_REPO_URL}")
