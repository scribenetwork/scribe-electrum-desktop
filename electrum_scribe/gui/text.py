...
from electrum_scribe.util import format_satoshis
from electrum_scribe.bitcoin import is_address, COIN, TYPE_ADDRESS
from electrum_scribe.transaction import TxOutput
from electrum_scribe.wallet import Wallet
from electrum_scribe.storage import WalletStorage
from electrum_scribe.network import NetworkParameters, TxBroadcastError, BestEffortRequestFailed
from electrum_scribe.interface import deserialize_server
...
class ElectrumGui:
    ...
    def __init__(self, config, daemon, plugins):
        ...
        if not storage.file_exists():
            print("Wallet not found. try 'electrum-scribe create'")
            exit()
    ...
    def do_send(self):
        if not is_address(self.str_recipient):
            self.show_message(_('Invalid Scribe address'))
            return
        try:
    ...
    def network_dialog(self):
        ...
        if out:
            ...
            if out.get('server') or out.get('proxy'):
                proxy = electrum_scribe.network.deserialize_proxy(out.get('proxy')) if out.get('proxy') else proxy_config
