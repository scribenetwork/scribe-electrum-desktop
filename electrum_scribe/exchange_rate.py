...
class ATAIX(ExchangeBase):
    async def get_rates(self, ccy):
        json = await self.get_json('api.ataix.com', '/api/prices/SCRIBE-%s' % ccy)
        return {ccy: Decimal(json['result'][0]['last'])}

class BitcoinAverage(ExchangeBase):
    async def get_rates(self, ccy):
        json1 = await self.get_json('apiv2.bitcoinaverage.com', '/indices/crypto/ticker/SCRIBEBTC')
        if ccy != "BTC":
...
class Bittrex(ExchangeBase):
    async def get_rates(self, ccy):
        json1 = await self.get_json('bittrex.com', '/api/v1.1/public/getticker?market=btc-scribe')
        if ccy != "BTC":
            json2 = await self.get_json('apiv2.bitcoinaverage.com', '/indices/global/ticker/BTC%s' % ccy)
...
class Bitbank(ExchangeBase):
    async def get_rates(self, ccy):
        json = await self.get_json('public.bitbank.cc', '/scribe_%s/ticker' % ccy.lower())
        return {ccy: Decimal(json['data']['last'])}

class Coincheck(ExchangeBase):
    async def get_rates(self, ccy):
        json = await self.get_json('coincheck.com', '/api/rate/scribe_%s' % ccy.lower())
...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coingecko.com',
                                   '/api/v3/simple/price?ids=scribe&vs_currencies=%s' % ccy)
        return {ccy: Decimal(json['scribe'][ccy.lower()])}
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/scribe/market_chart?vs_currency=%s&days=max' % ccy)
...
class CryptBridge(ExchangeBase):
    async def get_rates(self, ccy):
        json1 = await self.get_json('api.crypto-bridge.org', '/api/v1/ticker/SCRIBE_BTC')
        if ccy != "BTC":
            json2 = await self.get_json('apiv2.bitcoinaverage.com', '/indices/global/ticker/BTC%s' % ccy)
            return {ccy: Decimal(json1['last'])*Decimal(json2['last'])}
        return {ccy: Decimal(json1['last'])}
...
class Fisco(ExchangeBase):
    async def get_rates(self, ccy):
        json = await self.get_json('api.fcce.jp', '/api/1/last_price/scribe_%s' % ccy.lower())
        return {ccy: Decimal(json['last_price'])}

class NebliDex(ExchangeBase):
    async def get_rates(self, ccy):
        json = await self.get_json('www.neblidex.xyz', '/seed/?v=1&api=get_market_price&market=SCRIBE/%s' % ccy)
        return {ccy: Decimal(json)}

class TradeSatoshi(ExchangeBase):
    async def get_rates(self, ccy):
        json = await self.get_json('tradesatoshi.com', '/api/public/getmarketsummary?market=SCRIBE_BTC')
        return {'BTC': Decimal(json['result']['last'])}

class Zaif(ExchangeBase):
    async def get_rates(self, ccy):
        json = await self.get_json('api.zaif.jp', '/api/1/last_price/scribe_%s' % ccy.lower())
        return {ccy: Decimal(json['last_price'])}
