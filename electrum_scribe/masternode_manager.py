...
MASTERNODE_COLLATERAL_VALUE = 1000 * bitcoin.COIN
...
class MasternodeManager(object):
    ...
    def check_can_sign_masternode(self, alias):
        ...
        if uxto not in utxos:
            raise Exception('Masternode requires a 1000 SCRIBE collateral. Check if funds have been moved or spent.')
    ...
    def on_broadcast_announce(self, alias, r):
        ...
        if str(r).find("error") != -1:
            print(11111111111)
            err = str(r)
            raise Exception('Error response: %s' % str(err))

        if str(r).find("successful") != -1:
            result = str(r)
            mn_hash = mn.get_hash()
            if str(r).find(mn_hash) == -1:
    ...
    def masternode_subscription_response(self, response):
        ...
        if not result:
          return 
        collateral = response[0]
        mn = None
        print(555,collateral)
        for masternode in self.masternodes:
